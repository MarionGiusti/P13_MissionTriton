    ...
    <div class="mx-8">
      ...
      <v-row align="center">
        <v-col
          ...
          align="center"
        >
          <v-card class="mx-auto" flat color="#54658C">
            <v-img
              width="250px"
              height="250px"
            ...
            <v-expand-transition>
              <div v-show="show">
                ...
                <v-card-text>
                  ...
                  <br />
                  <span v-if="member.description"
                    >rôle: {{ member.description }}</span
                  >
                  <div>
                    <a
  ...
  methods: {
    async load_team() {
      ...
      const data = await getAPI.get(
        `/api/users/missionusers/get_team/?missionId=${miss}`
      );
      this.team = data.data;
    },
    ...
    async addMissionuser() {
      if (this.$refs.form.validate()) {
        try {
          ...
          this.load_team();
          alert(
            `L'utilisateur ${this.form.email} a bien été ajouté à la mission ${this.missionD.name} !`
          );
          this.resetForm();
        } catch (err) {
