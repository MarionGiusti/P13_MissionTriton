            ...
            <template v-slot:top>
              <v-dialog v-model="dialog" max-width="500px">
                <v-card>
                  ...
                  <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn color="teal" text @click="close" outlined>
                      Annuler
                    </v-btn>
                    <v-btn color="teal" text @click="save(editedItem)" outlined>
                      Modifier
                    </v-btn>
                  </v-card-actions>
                </v-card>
  ...
  methods: {
    ...
    async AddShipPosition() {
      if (this.$refs.form.validate()) {
        try {
          await getAPI.post("/api/missions/shippositions/", this.send_form, {
            headers: {
    ...
    async UpdateShipPosition(item) {
      try {
        await getAPI.patch(
          `api/missions/shippositions/${item.id}/`,
          {
