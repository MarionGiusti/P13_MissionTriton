...
export default new Vuex.Store({
  ...
  getters: {
    ...
    futureMissions: state => {
      return state.missions
        .filter(
          missions =>
            new Date(missions.start_date).getTime() > new Date().getTime()
        )
        .slice()
        .sort(
          (a, b) =>
    ...
    memberMission: (state, getters) => id => {
      ...
      return state.userDetails.missions
        ...
        .includes(getters.currentMission(id).name);
    }
  },
  ...
  mutations: {
    ...
    updateMissionUserDetails(state, data) {
      state.missionUserDetails = Object.assign(
        {},
        state.missionUserDetails,
        data
      );
    },
  ...
  actions: {
    ...
    async getListMissionUsers({ commit, state }, missionId) {
      await getAPI
        .get(`/api/users/missionusers/?missionid=${missionId}`, {
          headers: { Authorization: "Token " + state.token }
        })
        .then(data => {
    ...
    async patchMissionUserProfile({ commit, state }, missionusercredentials) {
      const { data } = await getAPI.patch(
        `api/users/missionusers/${state.missionUserDetails.id}/`,
        {
    ...
    async getShipPositionsMission({ commit }, missionId) {
      await getAPI
        .get(`/api/missions/shippositions/?missionid=${missionId}`)
        .then(data => {
    ...
    async postPost({ commit, state }, postcredentials) {
      const { data } = await getAPI.post("/api/posts/", postcredentials, {
        headers: {
